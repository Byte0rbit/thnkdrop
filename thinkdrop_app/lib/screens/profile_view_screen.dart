...
class ProfileViewScreen extends StatefulWidget {
  ...
  ProfileViewScreen({required this.user});

  @override
  _ProfileViewScreenState createState() => _ProfileViewScreenState();
}

class _ProfileViewScreenState extends State<ProfileViewScreen> with SingleTickerProviderStateMixin {
  String _firstName = '';
  String _lastName = '';
  String _bio = '';
  String _profession = '';
  ...
  Future<void> _toggleLike(int index, String visibility) async {
    final ideas = visibility == 'PUBLIC' ? _publicIdeas : _partialIdeas;
  ...
  // Load user profile from passed user object
  void _loadProfile() {
    final user = widget.user;
    print('Loading profile for user: ${user['username']}');
    setState(() {
      _firstName = user['first_name']?.toString() ?? '';
      _lastName = user['last_name']?.toString() ?? '';
      _bio = user['bio']?.toString() ?? '';
      _profession = user['profession']?.toString() ?? '';
      _socialLinks = (user['social_links'] as List<dynamic>?)?.map((link) {
      ...
      _profilePicUrl = (user['profile_pic'] != null && user['profile_pic'].isNotEmpty && user['profile_pic'].startsWith('http'))
          ? user['profile_pic']
          : user['profile_pic'] != null && user['profile_pic'].isNotEmpty
          ? '$rootUrl${user['profile_pic'].startsWith('/') ? user['profile_pic'].substring(1) : user['profile_pic']}'
          : '';
      _userId = user['id'] is int ? user['id'] : null;
  ...
  // Load ideas for public and partial visibility
  ...
  Future<void> _loadIdeasPage(String visibility) async {
    ...
    bool hasMore = visibility == 'PUBLIC' ? _publicHasMore : _partialHasMore;
    bool isLoadingMore = visibility == 'PUBLIC' ? _publicIsLoadingMore : _partialIsLoadingMore;
    ...
    try {
      final page = visibility == 'PUBLIC' ? _publicPage : _partialPage;
